...
const Bin = () => {
  ...
  const fetchFiles = async () => {
    try {
      setLoading(true);
      const res = await fetch('http://localhost:5000/cloud/bin', {
        credentials: 'include',
      });
  ...
  const restoreFile = async (fileId) => {
    try {
      const response = await fetch(`http://localhost:5000/cloud/restore/${fileId}/`, {
        method: 'PUT',
        credentials: 'include',
      });
  ...
  const deleteFilePermanently = async (fileId) => {
    try {
      const response = await fetch(`http://localhost:5000/cloud/deletefiles/${fileId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
