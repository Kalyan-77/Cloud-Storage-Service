...
const Sidebar = () => {
  ...
  const fetchStorageData = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/cloud/totalStorage', {
        credentials: 'include'
      });
  ...
  const handleFileChange = async (e) => {
    ...
    setLoading(true);
    try {
      await fetch('http://localhost:5000/cloud/upload', {
        method: 'POST',
        body: formData,
        credentials: 'include'
      });
    ...
    <div className="fixed top-0 left-0 h-screen w-72 bg-white border-r border-gray-200 flex flex-col shadow-sm">
      ...
      <nav className="flex-1 px-4 overflow-y-auto">
        ...
        <div className="mt-6 p-4 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border border-gray-200">
          ...
          <button 
            className="mt-3 w-full text-blue-600 hover:text-blue-700 text-sm font-medium py-1 transition-colors"
